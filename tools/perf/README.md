...
| GPU p50        | **within 10 % or 1.0 ms**, whichever is looser | The render cost. Every optimisation claim in a PR rests on this number. |
...
| `--runs <n>`         | `1`                | passes; the reported segment is the **median**  |
...
| `--strict`           | off                | exit 1 if the determinism check FAILs           |
...
```sh
npm run perf -- --runs 2 --label "aa=off (ships)" --ab "aa=legacy"
```
...
```sh
# Record a fresh baseline (two passes, median reported)
npm run perf -- --runs 2 --label baseline --baseline
...
npm run perf -- --runs 2 --label "aa=off" --ab "aa=legacy"

# How much does the scaler recover when it backs off a step?
npm run perf -- --runs 2 --res 1.5 --label "res=1.5" --ab "res=2"
